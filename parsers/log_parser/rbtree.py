...
class RbTree:
    ...
    def in_order_traversal(self, visit, kwargs):
        if self.root is not NIL:
            yield from self._in_order_traversal(self.root, visit, kwargs)

    def _in_order_traversal(self, node, visit, kwargs):
        if node is not NIL:
            yield from self._in_order_traversal(node.left, visit, kwargs)
            if kwargs:
                yield visit(node.data, **kwargs)
            else:
                yield visit(node.data)
            yield from self._in_order_traversal(node.right, visit, kwargs)
